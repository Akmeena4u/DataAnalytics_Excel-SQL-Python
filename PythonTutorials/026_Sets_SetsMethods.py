"""Python Sets : Sets are unordered collection of data items. They store multiple items in a single variable. Set items are separated by commas and enclosed within curly brackets {}. 
...
solution:- for creating empty set we needs to use - set()  ,, rather than using s={} , it will give dict"""

#Accessing set items: Using a For loop You can access items of set using a for loop
...
#Joining Sets :  We can perform operations like union and intersection on the sets just like in mathematics.
...
print(cities)                                 #output : Output: {'Berlin', 'Madrid', 'Tokyo', 'Delhi', 'Kabul', 'Seoul'} 
...
Set Methods : There are several in-built methods used for the manipulation of set.They are explained below
...
cities = {"Tokyo", "Madrid", "Berlin", "Delhi"}
cities.remove("Seoul")
print(cities)                            #Output: KeyError: 'Seoul'
...
cities = {"Tokyo", "Madrid", "Berlin", "Delhi"}
del cities
print(cities)              #Output: NameError: name 'cities' is not defined We get an error because our entire set has been deleted and there is no variable called cities which contains a set.
